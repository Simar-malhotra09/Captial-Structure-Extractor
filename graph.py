    ...
    for amt, group in by_amt.items():
        if len(group) > 1:
            ...
            for inst in group:
                ...
                inst["duplicate_flag"] = (
                    f"Same amount (${amt}mm) as: {'; '.join(others)}"
                )

    # Also check sums: if any pair of instruments sums to another instrument's amount
    all_amts = [
        (inst, round(inst["amount_mm"], 1))
        for inst in instruments
        if inst.get("amount_mm") and inst["amount_mm"] > 0
    ]
    amt_to_inst = {
        round(inst["amount_mm"], 1): inst
        for inst in instruments
        ...
        for j, (inst_b, amt_b) in enumerate(all_amts):
            ...
            if total in amt_to_inst:
                ...
                if inst_total is not inst_a and inst_total is not inst_b:
                    ...
                    inst_total["sum_flag"] = (
                        f"This amount (${total}mm) = {inst_a['label'][:30]} (${amt_a}mm) + {inst_b['label'][:30]} (${amt_b}mm)"
                    )
...
def parse_instruments(html: str, entities: list[str] = None) -> list[dict]:
    ...
    text_after_tables = re.sub(
        r"<table[^>]*>.*?</table>", " [TABLE] ", str(soup), flags=re.DOTALL
    )
    text_after_tables = BeautifulSoup(text_after_tables, "html.parser").get_text(
        separator=" "
    )
    ...
    for m in re.finditer(
        r"\((\d{1,2})\)\s*([A-Z].{20,800}?)(?=\(\d{1,2}\)\s*[A-Z]|$)", text_after_tables
    ):
        footnotes[m.group(1)] = re.sub(r"\s+", " ", m.group(2)).strip()
    # Letter footnotes: (a)...(b)...
    for m in re.finditer(
        r"\(([a-z])\)\s*([A-Z].{20,800}?)(?=\([a-z]\)\s*[A-Z]|$)", text_after_tables
    ):
        footnotes[m.group(1)] = re.sub(r"\s+", " ", m.group(2)).strip()
        ...
        for word in entity.split():
            if len(word) >= 4 and word.lower() not in (
                ...
                entity_patterns.append(
                    (entity, re.compile(r"\b" + re.escape(word) + r"\b", re.IGNORECASE))
                )

    def _strip_suffix(name: str) -> str:
        return re.sub(
            r",?\s*(Inc\.?|LLC|Ltd\.?|Corp\.?|Corporation|Company|Co\.|L\.P\.)$",
            "",
            name,
            flags=re.IGNORECASE,
        ).strip()
    ...
    for tidx, table in enumerate(soup.find_all("table")):
        for ridx, tr in enumerate(table.find_all("tr")):
            ...
            for cell in tr.find_all(["td", "th"]):
                for ix in cell.find_all("ix:nonfraction"):
                    ...
                    if raw and raw not in ("—", "\u2014", "no", ""):
                        try:
                            val_mm = round(
                                float(raw.replace(",", "")) * (10**scale) / 1e6, 3
                            )
                        except:
                            pass
                    elif raw in ("—", "\u2014"):
            ...
            if any(
                c in concepts
                for c in ("LineOfCredit", "LineOfCreditFacilityInterestRateAtPeriodEnd")
            ):
                ...
                "OperatingLeaseLiability" in concepts
                or "operating lease" in label_lower
            ):
            ...
            priority = None
            if "senior secured" in label_lower or inst_type in (
                "revolver",
                "term_loan",
                "credit_facility",
            ):
            ...
            all_amounts = [
                {
                    "concept": ix["concept"],
                    "value_mm": ix["value_mm"],
                    "ctx": ix.get("ctx", ""),
                }
                for ix in usd_ix
                if ix["value_mm"] is not None
            ]

            # Extract footnote references from label: (1), (2), (a), (b), etc.
            fn_refs = re.findall(r"\((\d{1,2})\)", label) + re.findall(
                r"\(([a-z])\)", label
            )
            resolved_fns = [
                f"({r}): {footnotes[r][:300]}" for r in fn_refs if r in footnotes
            ]
...
def parse_leases(html: str, parent_entity: str) -> list[dict]:
    ...
    # --- Finance leases ---
    if (
        "FinanceLeaseLiabilityCurrent" in all_lease_ix
        or "FinanceLeaseLiabilityNoncurrent" in all_lease_ix
    ):
        # Split available
            ...
            leases.append(
                {
                    ...
                    "concepts": ["FinanceLeaseLiabilityCurrent"],
                    "footnotes": [],
                    "source": "lease_note",
                }
            )
            ...
            leases.append(
                {
                    ...
                    "priority": "Senior Secured",
                    "entity": parent_entity,
                    "concepts": ["FinanceLeaseLiabilityNoncurrent"],
                    "footnotes": [],
                    "source": "lease_note",
                }
            )
    ...
    # --- Operating leases ---
    if (
        "OperatingLeaseLiabilityCurrent" in all_lease_ix
        or "OperatingLeaseLiabilityNoncurrent" in all_lease_ix
    ):
...
def deduplicate_leases(
    debt_instruments: list[dict], lease_instruments: list[dict]
) -> list[dict]:
    """
    ...
    finance_leases = [
        l for l in lease_instruments if "Finance" in l.get("concepts", [""])[0]
    ]
    operating_leases = [
        l for l in lease_instruments if "Operating" in l.get("concepts", [""])[0]
    ]

    finance_sum = round(
        sum(l["amount_mm"] for l in finance_leases if l["amount_mm"]), 1
    )
    operating_sum = round(
        sum(l["amount_mm"] for l in operating_leases if l["amount_mm"]), 1
    )
    ...
    if finance_sum_matched:
        print(
            f"  Dedup: finance lease sum ${finance_sum}mm matches debt table — skipping",
            file=sys.stderr,
        )
    if operating_sum_matched:
        print(
            f"  Dedup: operating lease sum ${operating_sum}mm matches debt table — skipping",
            file=sys.stderr,
        )
    ...
    for lease in lease_instruments:
        ...
        if amt in debt_amounts and amt > 0:
            print(
                f"  Dedup: {lease['label']} ${amt}mm exact match — skipping",
                file=sys.stderr,
            )
            continue
...
def match_entities_to_instruments(
    entities: list[str], instruments: list[dict], narrative: str
) -> dict[str, list[str]]:
    """
    ...
    for entity in entities_sorted:
        positions = [
            m.start() for m in re.finditer(re.escape(entity), narrative, re.IGNORECASE)
        ]
        ...
        for word in words:
            positions.extend(
                m.start()
                for m in re.finditer(
                    r"\b" + re.escape(word) + r"\b", narrative, re.IGNORECASE
                )
            )
    ...
    for inst in instruments:
        ...
        if not matched_entity and inst["type"] not in (
            "maturity_schedule",
            "total_or_subtotal",
            "transaction",
        ):
...
def build_mermaid(
    ...
    if unmatched:
        lines.append('    UNMATCHED["⚠ Unmatched"]')
...
"""
    ...
    for entity in entities:
        ...
        for inst in all_insts:
            ...
            avail = (
                f"{inst['amount_available_mm']:,.3f}"
                if inst.get("amount_available_mm")
                else ""
            )
    ...
    if corr_by_id:
        ...
        for cid, c in corr_by_id.items():
            changes = []
            if c.get("entity"):
                changes.append(f"entity → {c['entity']}")
            if c.get("priority"):
                changes.append(f"priority → {c['priority']}")
            if c.get("exclude"):
                changes.append("EXCLUDED")
            if c.get("clean_name"):
                changes.append(f"name → {c['clean_name'][:40]}")
            if c.get("issue_date"):
                changes.append(f"issued {c['issue_date']}")
            if c.get("amount_available_mm") is not None:
                changes.append(f"avail ${c['amount_available_mm']}mm")
            reason = c.get("reason", "")
...
def _find_narrative_snippet(instrument: dict, narrative: str) -> str:
    ...
    label_words = [
        ...
        if len(w) >= 5 and not w.replace(",", "").replace(".", "").isdigit()
    ]
    if label_words:
        for para in paragraphs:
...
def main():
    ...
    parser.add_argument(
        "input_dir",
        help="Directory containing debt_note.html, lease_note.html, balance_sheet.json",
    )
    parser.add_argument("--market-cap", type=float, default=0)
    parser.add_argument("-o", "--output", default=None)
    parser.add_argument(
        "--entities", default=None, help="Comma-separated entity names (skip NER)"
    )
    ...
    if lease_file:
        print(f", lease={os.path.basename(lease_file)}", file=sys.stderr, end="")
    if bs_file:
        print(f", bs={os.path.basename(bs_file)}", file=sys.stderr, end="")
    if meta_file:
        print(f", meta={os.path.basename(meta_file)}", file=sys.stderr, end="")
    ...
    if lease_file:
        ...
        print(
            f"Leases: {len(lease_instruments)} found, {len(new_leases)} new (not in debt table)",
            file=sys.stderr,
        )
    ...
    if bs_file:
        ...
        print(
            f"Balance sheet: cash=${bs_data['cash_mm']:,.1f}mm, NCI=${bs_data['nci_mm']:,.1f}mm",
            file=sys.stderr,
        )
    ...
    if api_key:
        ...
        try:
            ...
            print(
                f"  {n_corr} corrections, {n_guar} guarantor relationships",
                file=sys.stderr,
            )
            ...
            for c in llm_corrections.get("corrections", []):
                ...
                if c.get("priority"):
                    changes.append(f"priority→{c['priority']}")
                if c.get("amount_mm") is not None:
                    changes.append(f"amt→${c['amount_mm']}mm")
                if c.get("exclude"):
                    changes.append("EXCLUDE")
                if c.get("clean_name"):
                    changes.append(f"name→{c['clean_name'][:30]}")
                if c.get("amount_available_mm") is not None:
                    changes.append(f"avail→${c['amount_available_mm']}mm")
                conf = (
                    f" [{c.get('confidence', '')}]"
                ...
                print(
                    f"  [{c['id']}] {', '.join(changes)}{conf} | {c.get('reason', '')}",
                    file=sys.stderr,
                )

            for g in llm_corrections.get("guarantor_relationships", []):
                print(
                    f"  guarantor: {g.get('issuer', '')} → {g.get('guarantors', [])} ({g.get('guarantee_type', '')})",
                    file=sys.stderr,
                )
