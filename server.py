"""
FastAPI backend for capital structure extraction.

POST /api/extract     — upload files, starts extraction job
GET  /api/jobs/{id}   — poll job status + results
GET  /api/jobs/{id}/prompt — LLM prompt/approach text
...
from graph import (
    parse_instruments, parse_leases, parse_balance_sheet,
    deduplicate_leases, _preprocess_ixbrl, _flag_duplicate_amounts,
    _find_narrative_snippet, llm_validate, apply_corrections,
    build_mermaid, html_to_text
)
...
@app.get("/api/health")
def health():
    return {"status": "ok", "active": sum(1 for j in jobs.values() if j["status"] == "running")}
...
async def start_extraction(
    ...
    loop.run_in_executor(
        executor, _run_job, job_id, debt_html, lease_html, bs_json,
        annual_period, market_cap, api_key
    )
...
def get_source(job_id: str):
    ...
    for key, label in [("debt_html", "Debt Note"), ("lease_html", "Lease Note")]:
        ...
        if tables:
            parts.append(f'<h3 style="font-family:sans-serif;font-size:14px;margin:16px 0 8px;color:#333">{label} ({len(tables)} tables)</h3>')
            for tidx, table in enumerate(tables):
                ...
                table['style'] = table.get('style', '') + ';border:1px solid #ddd;margin-bottom:12px;'
                # Add row IDs
                for ridx, tr in enumerate(table.find_all('tr')):
                    tr['id'] = f"source-{prefix}-t{tidx}-r{ridx}"
                parts.append(str(table))
    
    html = '<div style="font-family:Times New Roman,serif;font-size:10pt;padding:8px">' + '\n'.join(parts) + '</div>'
...
async def chat_correction(job_id: str, req: ChatRequest):
    ...
    user_msg = json.dumps({
        ...
        "excluded": [{"id": i["id"], "label": i["label"], "amount_mm": i.get("amount_mm"), "reason": i.get("_reason", "")} 
    ...
    if corrections:
        ...
        for corr in corrections:
            ...
            if cid.startswith("new_"):
                ...
                if corr.get("exclude"):
                    # Move to excluded
                    job["result"]["instruments"] = [i for i in job["result"]["instruments"] if i["id"] != cid]
                    inst["_excluded"] = True
                    inst["_reason"] = corr.get("reason", "Removed by user")
                    job["result"]["excluded"].append(inst)
                else:
                    if corr.get("entity"): inst["entity"] = corr["entity"]
                    if corr.get("priority"): inst["priority"] = corr["priority"]
                    if corr.get("amount_mm") is not None: inst["amount_mm"] = corr["amount_mm"]
                    if corr.get("amount_available_mm") is not None: inst["amount_available_mm"] = corr["amount_available_mm"]
                    if corr.get("clean_name"): inst["clean_name"] = corr["clean_name"]
                ...
                if cid in excl_by_id:
                    ...
                    job["result"]["excluded"] = [i for i in job["result"]["excluded"] if i["id"] != cid]
        ...
        job["result"]["entities"] = [e for e in old_entities if e in entity_instruments] + new_entities
...
def _run_job(job_id, debt_html, lease_html, bs_json, annual_period, market_cap, api_key):
    job = jobs[job_id]
    job["status"] = "running"
    ...
    try:
        ...
        job["result"] = {
            "company_name": llm_corrections.get('company_name', parent or 'Unknown') if llm_corrections else (parent or 'Unknown'),
            ...
            "mermaid": mermaid,
            "guarantor_relationships": llm_corrections.get('guarantor_relationships', []) if llm_corrections else [],
